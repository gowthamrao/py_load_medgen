...
def test_apply_full_load_replicates_indexes(mocker):
    ...
    replicated_index_sql = "CREATE INDEX prod_table_name_idx ON prod_table_new USING btree (name);"
    execute_spy.assert_any_call(replicated_index_sql)

    # Also verify the atomic swap commands were called
    execute_spy.assert_any_call("ALTER TABLE IF EXISTS prod_table RENAME TO prod_table_old;")
